...
class ClaudeCodeService {
  ...
  async createSession(projectId, userId) {
    ...
    try {
      const workDir = path.join(
        Settings.claudeCode.workspaceBasePath,
        `workspace-${projectId}`,
      );
      ...
      ptyProcess.onExit(({ exitCode, signal }) => {
        ...
        this.destroySession(projectId).catch((error) => {
          logger.error(
            { projectId, error },
            "Error destroying terminal session after PTY exit",
          );
        });
      });
  ...
  async destroySession(projectId) {
    ...
    try {
      ...
      logger.error(
        { projectId, error },
        "Error destroying Claude Code session",
      );
    }
  }
    ...
    if (session) {
      ...
      if (session.connections.size === 0) {
        const timeout = setTimeout(() => {
          logger.info(
            { projectId },
            "No connections, destroying session after timeout",
          );
          this.destroySession(projectId).catch((error) => {
            logger.error(
              { projectId, error },
              "Error destroying terminal session after idle timeout",
            );
          });
        }, 300000); // 5 minutes
  ...
  async populateWorkspace(projectId, workDir) {
    ...
    const filesByPath =
      await ProjectEntityHandler.promises.getAllFiles(projectId);
  ...
  async syncProjectToWorkspace(session) {
    ...
    try {
      ...
      for (const projectPath of Array.from(session.docStates.keys())) {
        if (
          latestDocStates.has(projectPath) ||
  ...
  async removeDocFromWorkspace(session, projectPath) {
    ...
    await this.removeEmptyParentDirectories(
      session.workDir,
